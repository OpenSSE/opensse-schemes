...
public:
    ...
    void insert(const std::array<uint8_t, kTableKeySize>& key,
                const payload_type<PAGE_SIZE>             value);
...
private:
    struct Content
    {
        ...
        Content() = default;
        Content(std::array<uint8_t, kTableKeySize> k, payload_type<PAGE_SIZE> v)
...
OceanusServerBuilder<PAGE_SIZE>::OceanusServerBuilder(
    ...
    double             epsilon,
    size_t             max_search_depth)
    : data(), cuckoo_builder(max_n_elements, epsilon, max_search_depth),
      spilled_data(), max_n_elements(max_n_elements), db_path(db_path),
      n_elements(0)
{
