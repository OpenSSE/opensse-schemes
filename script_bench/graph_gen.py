...
def print_mean_var(stats, filepath):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", dest="in_file",nargs=1, required=True, help ="Input file")
    parser.add_argument("-o", "--out", dest="out_file", nargs=1, required=True, help ="Output file") 
    args = parser.parse_args()
    
    print "Parsing {0}, and outputting in {1}".format(args.in_file, args.out_file)
    stats = parse_file(args.in_file)
    means_var = compute_mean_var(stats)
    # print stats
    # print "\n\n"
    print_mean_var(means_var,args.out_file)
    print "Done"