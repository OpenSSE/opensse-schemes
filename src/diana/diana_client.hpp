...
template <typename T> class DianaClient {
public:
...
template <typename T> DianaClient<T>::~DianaClient() {}

template <typename T>
typename DianaClient<T>::keyword_index_type
DianaClient<T>::get_keyword_index(const std::string &kw) const {
...
template <typename T>
std::list<std::tuple<std::string, T, uint32_t>>
DianaClient<T>::get_counters_and_increment(
    const std::list<std::pair<std::string, index_type>> &update_list) {
  std::string keyword;
  index_type index;
...
template <typename T> size_t DianaClient<T>::keyword_count() const {
