...
namespace sse {
    namespace janus {
        ...
        crypto::Key<JanusClient::kPRFKeySize> JanusClient::insertion_derivation_master_key() const
        {
            return master_prf_.derive_key("add_derivation_master_key");
        }
        
        crypto::Key<JanusClient::kPRFKeySize> JanusClient::insertion_kw_token_master_key() const
        {
            return master_prf_.derive_key("add_kw_token_master_key");
        }
        
        crypto::Key<JanusClient::kPRFKeySize> JanusClient::deletion_derivation_master_key() const
        {
            return master_prf_.derive_key("del_derivation_master_key");
        }
        
        crypto::Key<JanusClient::kPRFKeySize> JanusClient::delertion_kw_token_master_key() const
        {
            return master_prf_.derive_key("del_kw_token_master_key");
        }
        
        JanusClient::JanusClient(const std::string& search_counter_map_path, const std::string& add_map_path, const std::string& del_map_path, crypto::Key<kPRFKeySize>&& master_key) :
            master_prf_(std::move(master_key)),
        ...
        SearchRequest JanusClient::search_request(const std::string &keyword)
        {
            ...
            crypto::PuncturableEncryption punct_encryption(punct_enc_master_prf_.derive_key(m_kw));
