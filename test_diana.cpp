...
void test_client_server()
{
    ...
    if (client_master_key_in.good() == true) {
        ...
        // generate new keys
        std::array<uint8_t, DianaClient<index_type>::kKeySize> master_derivation_key = sse::crypto::random_bytes<uint8_t, DianaClient<index_type>::kKeySize>();
        std::array<uint8_t, DianaClient<index_type>::kKeySize> kw_token_master_key = sse::crypto::random_bytes<uint8_t, DianaClient<index_type>::kKeySize>();

        
        // write keys to files
        
        ofstream client_master_key_out(client_master_key_path.c_str());
        client_master_key_out << std::string(master_derivation_key.begin(), master_derivation_key.end());
        client_master_key_out.close();

        ofstream client_kw_token_master_key_out(client_kw_token_master_key_path.c_str());
        client_kw_token_master_key_out << std::string(kw_token_master_key.begin(), kw_token_master_key.end());
